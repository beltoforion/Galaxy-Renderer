...
CumulativeDistributionFunction::CumulativeDistributionFunction()
    : m_pDistFun(nullptr), m_vM1(), m_vY1(), m_vX1(), m_vM2(), m_vY2(),
      m_vX2() {}
...
void CumulativeDistributionFunction::BuildCDF(int nSteps) {
  ...
  for (int i = 0; i < nSteps; i += 2) {
    x = (i + 2) * h;
    y += h / 3 *
         ((this->*m_pDistFun)(m_fMin + i * h) +
          4 * (this->*m_pDistFun)(m_fMin + (i + 1) * h) +
          (this->*m_pDistFun)(m_fMin + (i + 2) * h));
  ...
  if (m_vM1.size() != m_vX1.size() || m_vM1.size() != m_vY1.size())
    throw std::runtime_error(
        "CumulativeDistributionFunction::BuildCDF: array size mismatch (1)!");
